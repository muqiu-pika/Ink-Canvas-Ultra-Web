...
### 截屏与粘贴
- **区域截屏**：按 `Alt+C` 启动截屏，拖动选择区域后可选择插入到画板、保存到文件夹或加入照片列表
- **粘贴图片**：在放映/白板模式下按 `Ctrl+V` 粘贴剪贴板图片
- **全局粘贴**：按 `Ctrl+Shift+V` 进行全局粘贴

### 视频展台
- **设备管理**：侧栏设备列表中选择摄像头设备，画面自动插入白板
- **拍照功能**：支持拍照并将照片自动插入到白板新页面
- **画面旋转**：支持旋转摄像头画面和拍摄的照片
- **一键清除**：侧栏提供一键清除所有白板内容的按钮
- **自动管理**：退出/进入白板模式时自动停止/播放摄像头画面，减少资源消耗

### 数据迁移技巧
- **跨设备使用**：墨迹文件夹可移动到其他电脑正常使用原笔迹
- **备份恢复**：定期备份墨迹数据，防止意外丢失
- **版本兼容**：新版本软件兼容旧版本保存的墨迹数据
- **自动恢复**：静默重启或手动重启时会自动弹出是否恢复之前的内容
